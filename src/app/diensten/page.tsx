    ...
    <div className="bg-cream min-h-screen">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-16">
        ...
        <div className="mb-16 text-center">
          <p className="text-bronze text-xs font-mono uppercase tracking-widest mb-3">Wat wij bieden</p>
          <h1 className="font-serif text-navy text-4xl sm:text-5xl mb-5">Onze diensten</h1>
          <p className="text-ink-muted max-w-xl mx-auto leading-relaxed">
            Van recreatieve verhuur tot professioneel onderhoud — WielaardMarine is uw partner aan de Binnenmaas.
          </p>
        </div>
          ...
          {services.map((s, idx) => (
            <section key={s.id} id={s.id} className="scroll-mt-24">
              <div className={`grid grid-cols-1 lg:grid-cols-2 gap-10 items-start ${idx % 2 === 1 ? 'lg:[&>*:first-child]:order-2' : ''}`}>
                ...
                {/* Text */}
                <div>
                  <p className="text-bronze text-xs font-mono uppercase tracking-widest mb-2">{s.label}</p>
                  <h2 className="font-serif text-navy text-3xl mb-5">{s.title}</h2>
                  <div className="space-y-4">
                    {s.description.split('\n\n').map((p, i) => (
                      <p key={i} className="text-ink-muted leading-relaxed text-sm">{p}</p>
                    ))}
                  </div>
                  <Link
                    href={s.ctaHref}
                ...
                <div className="space-y-5">
                  <div className="bg-sand border border-navy/10 rounded-lg p-6">
                    <h3 className="text-bronze text-xs font-mono uppercase tracking-widest mb-4">Inbegrepen</h3>
                  ...
                  <div className="bg-navy rounded-lg p-6">
                    <h3 className="text-bronze text-xs font-mono uppercase tracking-widest mb-4">Indicatieve tarieven</h3>
                    <dl className="divide-y divide-cream/10">
                      {s.pricing.map(({ label, from }) => (
                        <div key={label} className="flex justify-between py-2.5 text-sm">
                          <dt className="text-cream/70">{label}</dt>
                          <dd className="text-cream font-medium">v.a. {from}</dd>
                        </div>
                      ))}
                    </dl>
                    <p className="text-cream/30 text-xs mt-4">
                      Exacte prijzen op aanvraag. Neem contact op voor een offerte op maat.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="mt-24 bg-navy rounded-lg p-10 text-center">
          <h2 className="font-serif text-cream text-2xl mb-3">Vragen of een offerte aanvragen?</h2>
          <p className="text-cream/60 mb-6 text-sm max-w-md mx-auto">
            Wij stellen graag een aanbod op maat samen voor uw verhuur, onderhoud of stalling.
          </p>
          <Link
            href="/contact"
